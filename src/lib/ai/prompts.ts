...
- **どこでその企業を知ったかを書かない。** 「法人情報で拝見し」「なび京都の事業者ページで」
  のように情報源を明かさない。名簿サイト経由だと伝えるのは相手に失礼にあたる。
- **「公開情報が少ない」「詳細が分からなかった」を連絡の理由にしない。**
  相手の情報発信が不足していると指摘することになる。触れられる事実がそれしか無い場合は、
  その事実そのもの（所在地・業種・年数など）に敬意をもって触れる。
- **日本語で書く。** 本文に英単語をそのまま混ぜない（DX・IT・SNS のような定着した略語は可）。
- **与えられたテーマを勝手に広げたり言い換えたりしない。** その企業がテーマに合わないと
  思った場合も、テーマ自体を書き換えてはいけない。`;
