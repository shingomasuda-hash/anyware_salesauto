...
- **既定で「採用ページを確認できた企業のみ」を表示します。** 全件を見たいときは
  フィルタの「採用ページなしも表示」にチェックを入れてください（CSV 出力にも同じ条件が適用されます）。
  採用ページの有無はクロールで実際にページを見つけたかどうかであり、AI の判定ではありません。
- **企業名をクリックするとその企業の公式サイトが開きます**（別タブ）。
  分析結果の詳細は企業名の右にある 📄 アイコンから開きます。
  公式サイトが未確認の企業だけ、企業名が詳細ページへのリンクになります。

### AI費用のコントロール

月あたりの Claude API 費用は、**「何社に AI を使うか」でほぼ決まります**。
仕組みは3段構えで、いずれもコードではなく環境変数で調整できます。

| 仕組み | 環境変数（既定） | 効果 |
| --- | --- | --- |
| 採用ページのある企業だけ分析する | `ANALYSIS_REQUIRE_RECRUIT_PAGE=true` | AI を呼ぶ社数が実測で 1/4 程度になる |
| Claude へ渡す本文を絞る | `ANTHROPIC_MAX_CONTEXT_CHARS=10000` | 入力トークン（費用の約半分）に直結 |
| 当月の費用上限 | `AI_MONTHLY_BUDGET_JPY=10000` | 上限到達で AI 分析を自動的に見送る |

**AI を使わずに済ませている判定**（費用ゼロ・クローラーのルールベース）:
採用ページの有無 / 問い合わせページ・フォーム / 公開メール / 電話 / SNS アカウント /
**営業拒否表記** / ページ種別の分類 / 公式サイトの確からしさ。
AI が担うのは「要約」「スコアリング」「営業トピックの提案」など、ルールで出せない部分だけです。

営業拒否の検出はクロール時点で完結するため、AI 分析を見送った企業でも
`sales_contact_allowed` が誤って「可」になることはありません（確認できなければ `unknown` のまま）。

予算に達すると **AI 分析だけ** が止まり、企業探索・クロール・重複排除は動き続けます。
現在の消化状況は `npm run ai:cost` の「当月のAI予算」で確認できます。

**月額費用は Claude だけではありません。** Web検索（Brave Search）は $5 / 1,000リクエストの従量課金で、
探索の規模に比例して増えます。`npm run discovery:verify` の「Web検索の利用」に
実リクエスト数・概算費用・月3,000社換算の見通しを出すので、両方を足して予算を判断してください
（GビズINFO は無料なので、Web検索を減らすほど総額は下がります）。
