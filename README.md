...
企業ごとに内容の違う営業文（メール下書き）を、**AI分析と同じ1回の Claude 呼び出しで**生成します。
別途 API を呼ばないため、費用の増分は出力トークンぶん（1社あたり 2円程度）だけです。

生成するには自社が何を売るのかを設定します。**未設定のあいだは生成しません**
（どの企業にも当てはまる一般的な文面を作らないため）。

```bash
SALES_SENDER_COMPANY=株式会社AnyWare
SALES_OFFERING_NAME=採用サイト改善支援
SALES_OFFERING_SUMMARY=中小製造業向けに、採用ページの改善とSNS運用の立ち上げを支援します
SALES_OFFERING_STRENGTHS=採用ページ制作|SNS運用代行|求人媒体の運用改善
SALES_OFFERING_CTA=30分ほどオンラインでお話をうかがえないでしょうか
```

自社サービスの定義は企業をまたいで変わらないため system プロンプト側に置いています。
Prompt Caching が効いたまま営業文を追加できます。

**機械的に止めていること**（`src/lib/ai/outreach.ts`）:

| 条件 | 動作 |
| --- | --- |
| 営業を断る表記が確認された企業 | 営業文を作成しない |
| 確認できていないメールアドレス・電話番号が文面にある | その営業文を破棄する |
| その企業固有の事実に一つも触れていない | その営業文を破棄する（テンプレート文面の混入防止） |

宛名は「ご担当者様」固定です。担当者名・連絡先を推測して書かせません。
営業可否が **未確認** の企業にも下書きは作りますが、画面に警告を出します。送信の判断は人が行ってください。

企業詳細ページの「営業文（下書き）」で、件名・本文・**その企業に合わせて触れた事実**を確認できます。
CSV にも「営業文件名」「営業文本文」列が入ります。一覧の「営業文あり」で絞り込めます。
