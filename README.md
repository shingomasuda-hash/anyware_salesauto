...
2. **ドメインの持ち主**（`checkDomainOwnership`）— そのドメインのトップページを取得し、
   社名が出てくるかを確認します。企業自身のサイトならトップページに社名が出ます。
   会員紹介ページなら団体・ポータルの名前になります。トップページを取得できないときは
   判断を保留し、落としません（取得失敗で実在する公式サイトを捨てないため）。
