...
│   │   │   ├── reconcile.ts        # 本人確認後のクロスソース照合（法人番号 / 公式ドメイン）
...
| **重複**（`duplicate_count`） | ① すでに `companies` に登録済みだった候補、② 同じ探索ラン内で別の情報源が同じ企業を指していた候補。どちらも **新規候補数には含みません** |
...
### 本人確認後の突き合わせ（クロスソース照合）

発見した時点では、GビズINFO の候補に **公式ドメインが無く**、Web検索の候補に **法人番号・住所が無い** ため、
「同じ会社を別の情報源が見つけていた」ことを判定できません。
そのため本人確認（公式サイト確認・法人番号照会）を終えた時点で **もう一度だけ突き合わせます**（`discovery/reconcile.ts`）。

- 突き合わせの根拠は **法人番号の一致** または **公式ドメインの一致** のみ（社名だけで統合しません）
- ドメイン一致は「法人番号が矛盾していない」かつ「社名が関連している」ときだけ採用します
  （公式サイトの誤判定を 2 社に伝播させないため）
- 残す側は `確認済 > スコア > 情報源の数 > 発見が早い` の順に決め、畳んだ側は `duplicate` として記録します
- 残した候補の `sources` に情報源を集約するため、`company_sources` に「どの情報源が何を観測したか」がすべて残ります

`discovery:verify` の「複数ソースで一致」は、この突き合わせまで含めた件数です。
**稼働している発見ソースが 1 つだけの場合は 0% になります**（精度の劣化ではありません）。
レポートは情報源ごとの稼働状況も出すので、0% の理由を取り違えずに読めます。
...
狙うのは次の3つです。**採用ページの有無では絞りません**（採用ページが無い企業は主要ターゲットのひとつ）。

| 区分 | 意味 |
| --- | --- |
| `no_recruit_page` 採用ページなし（採用の痕跡あり） | 求人媒体を使っている、または本文に採用の記載があるが、公式サイトに採用ページが無い |
| `weak_recruit_page` 採用ページが手薄 | 採用ページはあるが、募集要項・社員紹介・メッセージが不足している |
| `active_recruit` 採用に注力 | 採用ページが充実しており、採用に投資している |
| `no_signal` 採用の痕跡なし | 採用の記載が見つからない → **営業対象外**。一覧から除外し、AI 分析もしない |

判定は `src/lib/companies/recruit-target.ts` にあり、**クロールで取れた事実だけを使います（AI は使いません）**。
求人媒体（Indeed / リクナビ / マイナビ / doda / エン転職 / engage / Wantedly / ハローワーク 等）への
リンクと、本文・リンクテキストの採用表現から判定します。判断根拠は `recruit_target_reasons` に残るので、
人が検算できます。

### 企業一覧の見かた

営業に使えない企業は **既定で一覧・CSV から除きます**。外したいときは各チェックボックスで表示できます。

| 既定で除外するもの | 理由 | 外すチェック |
| --- | --- | --- |
| 営業を断っている企業（`sales_contact_allowed = false`） | 営業してはいけない | 営業不可も表示 |
| 公式サイトを確認できていない企業 | 企業名のリンク先が公式サイトにならない | 公式HP未確認も表示 |
| 採用の痕跡が無い企業（`no_signal`） | 営業ターゲットでない | 採用の痕跡なしも表示 |
| AI が判断しきれなかった企業（確度 30 未満 / ランクなし） | 営業判断に使えない | 確度が低い企業も表示 |

**「連絡可否」列の意味**:

| 表示 | 意味 |
| --- | --- |
| 可 | 問い合わせ・プライバシーページを読んだうえで、営業を断る表記が **無いことを確認できた** |
| 営業不可 | 「営業お断り」等の表記を **検出した**（既定で一覧に出ません） |
| 不明 | 表記が載りうるページをクロールできておらず、**確認できていない**（「可」と断定しない） |

「不明」は一覧に残します。確認できていないだけで断られたわけではないため、人が判断できるようにしています。
確認済みだけに絞るときは「営業可のみ（拒否・未確認を除外）」を使ってください。

- **企業名をクリックするとその企業の公式サイトが開きます**（別タブ）。分析結果は右の 📄 アイコンから。

### 企業ごとの文面（取材依頼 / サービス提案）

企業ごとに内容の違うメール下書きを、**AI分析と同じ1回の Claude 呼び出しで**生成します。
別途 API を呼ばないため、費用の増分は出力トークンぶん（1社あたり 2円程度）だけです。

目的を `SALES_OUTREACH_PURPOSE` で切り替えます。既定は **取材依頼**（`interview`）です。

```bash
# 取材依頼（既定）
SALES_OUTREACH_PURPOSE=interview
SALES_SENDER_COMPANY=株式会社AnyWare
SALES_INTERVIEW_TOPIC=中小製造業の採用・DXの取り組み
SALES_INTERVIEW_MEDIUM=自社メディアでの記事掲載
SALES_INTERVIEW_FORMAT=オンライン30分
```

**取材依頼ではサービスの説明・提案・売り込みを書きません。** 相手の課題を指摘したり
改善を提案したりもしません。「なぜこの企業に取材したいのか」を、その企業のサイトで
確認できた事実から述べるのが本体です。提案内容が決まってから
`SALES_OUTREACH_PURPOSE=proposal` に切り替え、`SALES_OFFERING_SUMMARY` 以下を設定してください。

未設定のあいだは生成しません（どの企業にも当てはまる一般的な文面を作らないため）。
取材依頼なら `SALES_INTERVIEW_TOPIC`、サービス提案なら `SALES_OFFERING_SUMMARY` が必要です。

依頼内容は企業をまたいで変わらないため system プロンプト側に置いています。
Prompt Caching が効いたまま文面を追加できます。

**機械的に止めていること**（`src/lib/ai/outreach.ts`）:

| 条件 | 動作 |
| --- | --- |
| 営業を断る表記が確認された企業 | 文面を作成しない（取材依頼でも作らない） |
| 確認できていないメールアドレス・電話番号が文面にある | その文面を破棄する |
| その企業固有の事実に一つも触れていない | その文面を破棄する（テンプレート文面の混入防止） |

宛名は「ご担当者様」固定です。担当者名・連絡先を推測して書かせません。
営業可否が **未確認** の企業にも下書きは作りますが、画面に警告を出します。送信の判断は人が行ってください。
...
Verification Score が 60〜79点の候補は、以前は `/review` で1件ずつ承認する設計でした。
実データでは半数近くがこの帯に入るため、**自動化が人の作業速度で止まる**状態でした。
判断を1つに絞って自動化しています。

| 状態 | 判断 |
| --- | --- |
| 公式サイトを確認できている（信頼度60点以上） | 企業リストに追加（社名・法人番号などの加点が足りないだけ） |
| 公式サイトを断定できない / 無い | 見送り（そもそも一覧に出せないため、人が見ても結論は同じ） |
| 求人媒体・法人情報DB・団体ドメインのURL | 見送り（信頼度に関わらず通しません） |

この結果、`companies` に入る企業は必ず「自社ドメインの公式サイトを確認済み」になります。
自動で通した候補は `reviewed_by = 'auto'` として記録するので、後から追えます。
人が確認する運用に戻すには `DISCOVERY_AUTO_REVIEW=false` を設定してください。

既に溜まっている確認待ちは `npm run maintain` が片付けます（単体なら `npm run db:auto-review`）。
...
4. 確認待ちの候補の自動判定（`db:auto-review`）
5. 公式サイトURLの再点検（`db:recheck-sites`）
6. 採用状況が未判定の企業の再クロール投入（`recrawl -- --missing-target`）
7. キューの処理（`jobs:run -- --drain`）— `--apply` のときだけ

AI 分析には実費がかかるため `maintain` では投入しません。必要なときだけ `npm run reanalyze` を使ってください。
以下は各ステップを単体で実行したい場合の説明です。

#### 公式サイトの判定（ドメインの持ち主で判断する）

商工会議所の会員紹介ページ・地域ものづくりポータル・求人媒体の企業ページは、
**社名・所在地・電話番号がすべて載っている**ため、ページ単体の照合では公式サイトと区別できません。
ドメインを1件ずつ除外リストに足していく方式では追いつかないため、次の2段で判断します。

1. **URL の形**（`rejectOfficialSiteUrl`）— 除外ドメイン・法人番号13桁・名簿ページのパス・
   団体専用ドメイン（`.or.jp` / `.gr.jp` / `.ac.jp` / `.ed.jp`。株式会社は登録できません）
2. **ドメインの持ち主**（`checkDomainOwnership`）— トップページに社名が出るかを確認します。
   グループ会社は親のドメイン（`nagatsu-g.co.jp`）を使うため、社名の「核」でも照合します
   （長津製作所 → 長津 / アオイ自動車工業 → アオイ）。

   **社名が見つからないだけでは落としません。** 社名を画像で出している自社サイトを
   落としてしまうためです（実データで3社を誤って外しました）。落とすのは、名簿・ポータルだと
   言える根拠があるときだけです。

   - `looksLikeRecordPageUrl` — 1社ごとに識別子を振るページ（`/company/4630/`
     `/company/b0140/` `?n=4644`）。自社サイトは `/company` `/about` のような固定の区画を使う
   - `looksLikeCompanyDirectoryPage` — トップページに多数の企業が並んでいる

   トップページを取得できないときも判断を保留します（取得失敗で実在する公式サイトを捨てないため）。

公式サイト判定の信頼度は、**実データで取れる加点だけで閾値に届くよう**に組んでいます。
以前は Web検索で見つけた正しい公式サイトの満点が 55点で、閾値60点に構造的に届きませんでした。

| 加点 | 点数 | 実データでの取得可否 |
| --- | --- | --- |
| 出所（GビズINFO 55 / Google Places 35 / Web検索 15） | 15-55 | ○ |
| ページタイトル・本文に社名 | 12-20 | ○ |
| 所在地が一致 | 15 | ○ |
| 会社概要の記載あり | 5 | ○ |
| **ドメインの持ち主が同社と確認** | **10** | ○（トップページの名義で裏を取る） |
| 電話番号が一致 | 15 | **×** 候補データの保有率0%（GビズINFOは電話番号を返さない） |
| 法人番号が一致 | 20 | △ ページに載っていることは稀 |
| ドメイン名が会社名に類似 | 12 | **×** 英字のみ判定のため日本語社名では常に0 |

Web検索経由でも「社名一致＋所在地一致＋持ち主の確認」で 65点になり合格します。
所在地が一致しない場合は 50点で不合格のままです（別会社のサイトを採用しない）。
持ち主の確認が「保留」（トップページを取得できない等）の場合は加点しません。
否定できなかっただけのポータルに加点してしまうためです。

判定を厳しくすると、前の基準で外した企業が取り残されます。
`npm run db:recheck-sites -- --apply` は**外すだけでなく、現在の基準では問題ない URL を戻します**
（`website_candidates` に理由つきで残してあるため）。戻すのはクロール待ちの状態までで、
「確認済み」にはしません。公式サイトの確定はクロールの判定に任せます。

判定は `scoreOfficialSiteCandidate` の内部で必ず通るため、呼び出し側が忘れることはありません。
**GビズINFO に登録されている URL も例外にしません**（誤った URL が登録されている企業があり、
出所の基礎点だけで閾値を超えて公式サイト扱いになっていました）。
...
判定を追加・変更しても既存の企業には反映されません。再クロールが必要です。
クロール自体に API 費用はかかりません（公式サイトは特定済みなので Web 検索も走りません）。

```bash
npm run recrawl                               # 対象を表示するだけ
npm run recrawl -- --missing-target --apply   # 採用状況が未判定の企業だけ
npm run recrawl -- --prefecture 京都府 --apply
npm run jobs:run -- --drain
```

AI 分析は既定では走りません。同時に分析し直すときだけ `--with-analysis` を付けてください（費用がかかります）。
...
AI が「判断材料が足りない」と判定した企業は、リストに載せても営業判断に使えません。
既定で **一覧・CSV から除外** します（`MIN_ANALYSIS_CONFIDENCE = 30`、`src/lib/companies/constants.ts`）。

| 状態 | 一覧に出るか |
| --- | --- |
| 分析済み・確度 30 以上・ランクあり | 出る |
| 分析済み・確度 30 未満 | **出ない** |
| 分析済み・スコアが1つも出ずランクなし | **出ない** |
| まだ分析していない（AI予算の上限に達した分など） | 出る |

未分析の企業を除外しないのは、機械抽出の情報（採用ページ・問い合わせ先・SNS・営業拒否）で
絞り込めるためです。「分析したが判断できなかった」ことと「まだ分析していない」ことは別に扱います。

除外された企業もデータは残っています。一覧のフィルタ「確度が低い企業も表示」で確認でき、
条件を変えて再分析（`npm run reanalyze`）すれば確度が上がる場合があります。
除外が多いときは `ANTHROPIC_MAX_CONTEXT_CHARS` / `ANTHROPIC_EFFORT` を上げると判断材料が増えます
（費用は上がります）。`npm run discovery:verify` に「確度不足で除外」の件数が出ます。

### AI予算が上限に達したときの動き

`AI_MONTHLY_BUDGET_JPY` に達すると AI 分析だけが止まります。
このとき **どの企業が分析されるか** が結果を左右するため、クロールで機械的に取れた事実から
分析の順番をつけています（`src/lib/jobs/analysis-priority.ts`）。AI は順番付けには使いません。

採用ページの充実度 / 連絡手段の数 / SNS / 情報量が多い企業から先に分析され、
**営業を断る表記がある企業は最後**になります。
予算内に収まらなかった企業も、機械抽出の情報（採用ページ・問い合わせ先・SNS・営業拒否）は
そろっているため、絞り込みには使えます。

### リスト作成だけを行う（AI費用をかけない）

`.env.local` に次を設定すると、**AIを一切呼びません**。
...
- 企業の探索（GビズINFO / Google Places / Web検索）
- 公式サイトの特定と確認
- クロール（連絡先・問い合わせフォーム・SNSの取得）
- 採用状況の判定（`recruit_target`。クロール結果から機械的に判定するためAIは不要）
- 一覧・絞り込み・CSV出力

止まるもの: AI分析（営業ランク・確度）と取材依頼文の生成。

費用は Brave Search のみ（Web検索を使う場合、1,000リクエスト5USD）。
GビズINFO・Google Places・クロールは無料です。

分析したくなったら `AI_ANALYSIS_ENABLED=true` に戻し、`npm run reanalyze -- --apply`
→ `npm run jobs:run -- --drain` で後からまとめて分析できます。
**リストを作り直す必要はありません。**

判定は `src/lib/ai/gate.ts` の1か所だけで行うため、探索・再クロール・Vercel Cron の
どの経路から来ても止まります。設定を切り替えた後にキューへ残っていたジョブも実行しません。

### 文面の確認

フォームに入力する前に、実際に送られる文面をターミナルで読めます（DBは変更しません）。

```bash
npm run outreach:show                # 直近の3社
npm run outreach:show -- --limit 10
```

件名・本文に加えて、**その企業固有として触れた事実**と**推測が含まれる箇所**を表示します。
固有の事実が0件のものはテンプレート文面の可能性があるため、そこを見て判断できます。

文面を変えるとき:

| 変えたいもの | 直す場所 |
| --- | --- |
| トーン・構成・長さ | `src/lib/ai/prompts.ts` の取材依頼の指示 |
| 取材テーマ・掲載先・形式 | `.env.local` の `SALES_INTERVIEW_TOPIC` / `_MEDIUM` / `_FORMAT` |
| 差出人 | `.env.local` の `SALES_SENDER_*` |

変更後は `npm run reanalyze -- --limit 3 --apply && npm run jobs:run -- --drain` で作り直します
（1社あたり約14円かかります）。

### 営業フォームへの自動入力（送信は人が行う）

各企業のフォームを開いて送信直前まで入力します。**送信は絶対に行いません。**

```bash
npm run outreach                       # 対象企業と入力できる項目を表示するだけ
npm run outreach -- --open --limit 3   # ブラウザを開いて実際に入力する
```
...
流れは、フォームを開く → 項目の意味を判定して入力 → 緑の枠で入力箇所を示す →
入力できなかった項目を一覧表示 → **人が内容を確認して送信ボタンを押す** →
ターミナルで結果（送信した / 送らない / 保留）を記録、です。

安全のための制約:

- **送信ボタンは押しません**（クリックもしません）
- **同意チェックは自動で入れません**。同意は人が行う行為のため
- **差出人情報が未設定の項目は入力しません**。架空の連絡先を送らないため
  （`SALES_SENDER_*` を `.env.local` に設定してください）
- 営業を断っている企業（`sales_contact_allowed = 'false'`）は対象にしません
- **問い合わせフォームが公式サイトと同じドメインにある企業だけ**を対象にします。
  企業ディレクトリ・掲載サイトのフォームは、その企業のものではないため送りません
  （実データで、なび京都・ツクリンク・法人情報DB のフォームを開いてしまいました）
- 本文が入力欄の文字数上限を超える場合は、切り詰めずに人へ判断を委ねます
- 意味が判断できない項目は空欄のまま人に提示します（勝手に埋めません）

項目の意味は `name`・`id`・ラベル文字・`placeholder`・入力型を合わせて判定します
（`src/lib/outreach/form-fill.ts`）。判定順に意味があり、フリガナは氏名より先、
確認用メールは通常のメールより先に見ます。日本語フォームの表記ゆれはテストで固定しています。

記録は `companies.outreach_status` に入ります（`unsent` / `opened` / `sent` / `skipped` / `failed`）。
一度送った企業は次回の対象になりません。
...
npm run db:reverify        # 候補URLを持つ既存候補を現在の基準でやり直す（Web検索は走らない＝費用ゼロ）
